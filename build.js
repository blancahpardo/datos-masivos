...
.tema-title { font-size:19px; font-weight:bold; color:var(--azul); margin-top:6px; font-family:Georgia,'Times New Roman',serif; }
.tema-case { font-size:11.5px; color:var(--gris); font-style:italic; opacity:.85; }
.tema-btn { position:relative; }
.tema-btn::before { content:""; position:absolute; top:0; left:18px; width:34px; height:10px; background:var(--humo); opacity:.55; border-radius:0 0 3px 3px; }
...
.im-chapter { animation:imFadeIn .25s ease; max-width:740px; }
.im-sub-body > .im-block-p:first-child::first-letter { font-family:Georgia,'Times New Roman',serif; font-size:2.6em; font-weight:bold; float:left; line-height:.85; padding:4px 6px 0 0; color:var(--amar); }
.im-reveal-more-toggle { display:inline-flex; align-items:center; gap:6px; background:#fff; border:1.5px dashed var(--humo); color:var(--azul); font-size:12.5px; font-weight:bold; border-radius:20px; padding:8px 16px; cursor:pointer; font-family:Arial,Helvetica,sans-serif; margin:2px 0 18px; }
.im-reveal-more-toggle:hover { border-color:var(--azul); background:var(--claro); }
.im-reveal-more-box { display:none; }
.im-reveal-more-box.open { display:block; animation:imFadeIn .25s ease; }
...
.im-block-h4 { font-size:15px; font-weight:bold; color:var(--azul); margin:20px 0 8px; }
.im-sub-body .im-block-h4:first-child { margin-top:0; }
...
.im-table-bare { overflow-x:auto; margin:0 0 16px; border-radius:8px; box-shadow:0 2px 10px rgba(159,177,186,.28); scrollbar-width:thin; scrollbar-color:var(--humo) transparent; }
.im-table-bare::-webkit-scrollbar, .im-code-box::-webkit-scrollbar, .im-modal table::-webkit-scrollbar { height:6px; }
.im-table-bare::-webkit-scrollbar-thumb, .im-code-box::-webkit-scrollbar-thumb { background:var(--humo); border-radius:3px; }
.im-table-bare::-webkit-scrollbar-track, .im-code-box::-webkit-scrollbar-track { background:transparent; }
...
.im-modal { background:#fff; border-radius:14px; max-width:760px; width:100%; max-height:90vh; min-width:0; display:flex; flex-direction:column; box-shadow:0 12px 40px rgba(0,0,0,.3); }
...
.im-modal-body { flex:1; min-height:0; min-width:0; overflow:auto; padding:16px 20px 20px; text-align:center; }
.im-modal-body table { min-width:max-content; }
.im-figure { margin:6px 0 20px; }
.im-figure-img { display:block; width:auto; max-width:100%; height:auto; max-height:82vh; margin:0 auto; border-radius:10px; box-shadow:0 4px 16px rgba(159,177,186,.35); cursor:zoom-in; }
...
.im-term-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(200px,1fr)); gap:14px; margin:4px 0 20px; }
.im-term-card { background:var(--claro); border:1.5px dashed var(--humo); border-radius:8px; padding:16px 18px; position:relative; transition:transform .15s ease, box-shadow .15s ease; }
.im-term-card:hover { transform:translateY(-2px); box-shadow:3px 5px 0 rgba(30,24,18,.12); }
.im-term-card::before { content:""; position:absolute; top:0; left:14px; width:26px; height:7px; background:var(--humo); opacity:.5; border-radius:0 0 3px 3px; }
.im-term-card .im-term-title { display:block; font-family:Georgia,'Times New Roman',serif; font-weight:bold; color:var(--azul); font-size:15px; margin:6px 0 6px; }
.im-term-card .im-term-desc { font-size:13px; color:var(--gris); line-height:1.6; }
...
function copyrightFooter(temaLabel) {
  return `© Dra. Blanca Hernández Pardo · Datos Masivos${temaLabel ? ' · ' + temaLabel : ''}<br>Queda prohibida la difusión, distribución o reproducción total o parcial de este material sin autorización expresa de la autora.`;
}
...
function buildImageView(viewKey, tema) {
  ...
  ${cfg.captionHtml ? `<p class="img-caption">${cfg.captionHtml}</p>` : ''}
</div>`;
}
...
function buildInteractiveResourceView(viewKey, tema) {
  const cfg = tema.views[viewKey];
  if (!cfg || !cfg.exists) return '';
  const label = viewLabel(viewKey, tema);
  return `<div id="view-${viewKey}" class="view sub-view fullbleed" hidden>
...
function buildInteractiveManualView(viewKey, tema) {
  const cfg = tema.views[viewKey];
  if (!cfg || !cfg.exists) return '';
  // Most topics have a single canonical PDF under the 'manual' key. A topic
  // with more than one interactive manual sharing one directory (each needs
  // to link to its own PDF instead of a single shared tema.views.manual.file)
  // sets pdfViewKey on each one to point at its own PDF.
...
<title>${esc(tema.titleFull)} — Datos Masivos</title>
...
(function() {
  ...
  function imFormatBold(s) {
    var out = "", i = 0, bold = false, ital = false;
    while (i < s.length) {
      if (s.charAt(i) === "*" && s.charAt(i + 1) === "*") { out += bold ? "</strong>" : "<strong>"; bold = !bold; i += 2; }
      else if (s.charAt(i) === "*") { out += ital ? "</em>" : "<em>"; ital = !ital; i += 1; }
      else { out += s.charAt(i); i += 1; }
    }
    return out;
  }
  ...
  function imOpenModal(title, bodyHtml, wide) {
  ...
  function imRenderBlocks(blocks, keyPrefix) {
    ...
    blocks.forEach(function(b, i) {
      ...
      if (b.type === "p") html += '<p class="im-block-p">' + imFormatInline(b.text) + "</p>";
      else if (b.type === "h4") html += '<h4 class="im-block-h4">' + imFormatInline(b.text) + "</h4>";
        ...
        '<template data-im-img-src="' + key + '"><img src="' + imEsc(b.src) + '" alt="' + imEsc(b.alt || "") + '">' +
        (b.caption ? '<figcaption class="im-figure-caption">' + imFormatInline(b.caption) + "</figcaption>" : "") + "</template>";
      ...
      else if (b.type === "term_card_group") html += '<div class="im-term-grid">' + b.items.map(function(it) {
        return '<div class="im-term-card"><span class="im-term-title">' + imFormatInline(it.term) + '</span><span class="im-term-desc">' + imFormatInline(it.desc) + "</span></div>";
      }).join("") + "</div>";
      else if (b.type === "reveal_group") html += '<button class="im-reveal-more-toggle" data-im-reveal-more="' + key + '">' + (b.label || "Seguir leyendo ↓") + '</button>' +
        '<div class="im-reveal-more-box" id="im-reveal-more-' + key + '">' + imRenderBlocks(b.blocks, key) + "</div>";
    });
    return html;
  }
  function imWireCommon(root, data) {
    ...
    root.querySelectorAll("[data-im-reveal-more]").forEach(function(btn) {
      btn.addEventListener("click", function() {
        var box = document.getElementById("im-reveal-more-" + btn.dataset.imRevealMore);
...
function buildIndexHtml(temas) {
  const buttons = temas.map(t => `<a class="tema-btn" data-tema="${t.dir}" href="${t.dir}/index.html" style="display:none">
    ...
    ${t.caseLabel ? `<span class="tema-case">${esc(t.caseLabel)}</span>` : ''}
  </a>`).join('\n  ');
  const teacherHash = sha256(TEACHER_PASSWORD);
...
<div id="gate">
  <div class="box">
    <div class="kicker">Datos Masivos</div>
    <h1>Expediente cerrado</h1>
    <p>Contenido protegido. Introduce la contraseña facilitada en clase.</p>
    <input type="password" id="pw" placeholder="Contraseña" autofocus>
    <button class="submit" id="enter">Acceder</button>
...
<div id="page">
<header>
  <div class="kicker">Datos Masivos · Grado en Tecnologías del Lenguaje</div>
  <h1>Archivo de la asignatura</h1>
  <p class="author">Dra. Blanca Hernández Pardo</p>
  <p>Siete expedientes, siete unidades. Elige uno para abrir su carpeta.</p>
</header>
...
const TEMAS = [
  { dir: 'u0', numLabel: 'UNIDAD 0', titleShort: 'Presentación', titleFull: 'Unidad 0 · Presentación y ecosistema de herramientas', kicker: 'Unidad 0 · Presentación y ecosistema de herramientas',
    viewOrder: ["manual_interactivo", "principal", "practica", "mp5"],
    views: {
      ...
      principal: { exists: true, kind: 'download', label: 'Cuaderno principal guiado', icon: '📘',
  ...
  { dir: 'u2', numLabel: 'UNIDAD 2', titleShort: 'Preparación y limpieza', titleFull: 'Unidad 2 · Preparación, limpieza y transformación de datos', kicker: 'Unidad 2 · Preparación, limpieza y transformación de datos',
    ...
    views: {
      ...
      evaluable: { exists: false },
      practica: { exists: true, items: [{ id: 'unica', label: 'Práctica', file: 'practica.pdf' }],
        extraDownload: { file: 'materiales_practica.zip', label: 'Descargar el material de la práctica (JSON de tweets brutos)' } },
      mp5: { exists: true, kind: 'interactive_5mp', label: 'Reto relámpago', icon: '⚡',
        desc: 'Cuestionario interactivo de repaso, a pantalla completa', dataFile: '5mp.json' },
    } },
  { dir: 'u3', numLabel: 'UNIDAD 3', titleShort: 'Exploración visual', titleFull: 'Unidad 3 · Exploración y visualización de datos masivos', kicker: 'Unidad 3 · Exploración y visualización de datos masivos',
    caseLabel: 'Caso: Mirador DataLab',
    viewOrder: ["video_historia", "manual_interactivo", "video_teoria", "recurso", "principal", "practica", "mp5"],
    views: {
      manual: { exists: true, file: 'manual.pdf' },
      manual_interactivo: { exists: true, kind: 'interactive_manual', label: 'Manual teórico', icon: '✨',
        desc: 'Manual interactivo, navegable por paneles', dataFile: 'manual_interactivo.json' },
      ...
      video_teoria: { exists: true, kind: 'video', label: 'Vídeo: marco teórico', icon: '🎬',
        desc: 'Resumen narrado del marco teórico de la unidad', file: 'video_teoria.mp4' },
      recurso: { exists: true, kind: 'interactive_resource', label: 'El Mirador', icon: '🕵️',
        desc: 'Estación de repaso interactiva, en el navegador', file: 'recurso.html' },
      principal: { exists: true, file: 'cuaderno_principal.ipynb' },
      evaluable: { exists: false },
      practica: { exists: true, items: [{ id: 'unica', label: 'Práctica', file: 'practica.pdf' }],
        extraDownload: { file: 'materiales_practica.zip', label: 'Descargar el material de la práctica (CSV de tweets preparados)' } },
      mp5: { exists: true, kind: 'interactive_5mp', label: 'Reto relámpago', icon: '⚡',
        desc: 'Cuestionario interactivo de repaso, a pantalla completa', dataFile: '5mp.json' },
    } },
  { dir: 'u4', numLabel: 'UNIDAD 4', titleShort: 'Corpus documentales', titleFull: 'Unidad 4 · Corpus y colecciones documentales extensas', kicker: 'Unidad 4 · Corpus y colecciones documentales extensas',
    ...
    views: {
      ...
      evaluable: { exists: false },
      practica: { exists: true, items: [{ id: 'unica', label: 'Práctica', file: 'practica.pdf' }],
        extraDownload: { file: 'materiales_practica.zip', label: 'Descargar los materiales de la práctica (colección de recortes)' } },
      mp5: { exists: true, kind: 'interactive_5mp', label: 'Reto relámpago', icon: '⚡',
        desc: 'Cuestionario interactivo de repaso, a pantalla completa', dataFile: '5mp.json' },
    } },
  { dir: 'u5', numLabel: 'UNIDAD 5', titleShort: 'Bases de datos', titleFull: 'Unidad 5 · Almacenamiento y acceso a datos a gran escala', kicker: 'Unidad 5 · Almacenamiento y acceso a datos a gran escala',
    ...
    views: {
      ...
      video_teoria: { exists: true, kind: 'video', label: 'Vídeo: marco teórico', icon: '🎬',
        desc: 'Resumen narrado del marco teórico de la unidad', file: 'video_teoria.mp4' },
      recurso: { exists: true, kind: 'interactive_resource', label: 'El Expediente', icon: '🕵️',
        desc: 'Estación de repaso interactiva, en el navegador', file: 'recurso.html' },
      principal: { exists: true, file: 'cuaderno_principal.ipynb' },
      evaluable: { exists: false },
      practica: { exists: true, items: [{ id: 'unica', label: 'Práctica', file: 'practica.pdf' }],
        extraDownload: { file: 'materiales_practica.zip', label: 'Descargar los materiales de la práctica (glosario y base de datos)' } },
      mp5: { exists: true, kind: 'interactive_5mp', label: 'Reto relámpago', icon: '⚡',
        desc: 'Cuestionario interactivo de repaso, a pantalla completa', dataFile: '5mp.json' },
    } },
  { dir: 'u6', numLabel: 'UNIDAD 6', titleShort: 'Automatización', titleFull: 'Unidad 6 · Automatización y comunicación de resultados', kicker: 'Unidad 6 · Automatización y comunicación de resultados',
    caseLabel: 'Caso: Bitácora de Datos',
    viewOrder: ["video_historia", "manual_interactivo", "video_teoria", "recurso", "principal", "practica", "mp5"],
    views: {
      manual: { exists: true, file: 'manual.pdf' },
      manual_interactivo: { exists: true, kind: 'interactive_manual', label: 'Manual teórico', icon: '✨',
      ...
      video_teoria: { exists: true, kind: 'video', label: 'Vídeo: marco teórico', icon: '🎬',
        desc: 'Resumen narrado del marco teórico de la unidad', file: 'video_teoria.mp4' },
      recurso: { exists: true, kind: 'interactive_resource', label: 'La Rotativa', icon: '🕵️',
        desc: 'Estación de repaso interactiva, en el navegador', file: 'recurso.html' },
      principal: { exists: true, file: 'cuaderno_principal.ipynb' },
      evaluable: { exists: false },
      practica: { exists: true, items: [{ id: 'unica', label: 'Práctica', file: 'practica.pdf' }],
        extraDownload: { file: 'materiales_practica.zip', label: 'Descargar el material de la práctica (CSV de neologismos de marzo)' } },
      mp5: { exists: true, kind: 'interactive_5mp', label: 'Reto relámpago', icon: '⚡',
        desc: 'Cuestionario interactivo de repaso, a pantalla completa', dataFile: '5mp.json' },
    } },
];
